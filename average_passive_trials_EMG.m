...
function [output_array] = average_passive_trials_EMG(varargin)
    ...
    if nargin == 18
    %% two trials submitted, to be averaged %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
        force1 = varargin{1};
        gonio1 = varargin{2};
        %angle1 = varargin{3};
        displ1 = varargin{4};
        ...
        if min(gonio_new1) > 0.0
            ...
            coeffs_force = gonio_modified\force1(1:loc_angle_end); % NB backslash --> slope
            coeffs_torque = gonio_modified\torque1(1:loc_angle_end);
            ...
            force1 = [coeffs_force(1); (angle_half*coeffs_force(2)) + coeffs_force(1); force1];
        ...
        if min(gonio_new2) > 0.0
            ...
            coeffs_force = gonio_modified\force2(1:loc_angle_end); % NB backslash --> slope
            coeffs_displ = gonio_modified\displ2(1:loc_angle_end);
            coeffs_torque = gonio_modified\torque2(1:loc_angle_end);
            % enlarge arrays by adding values at zero angle to the front
            angle_half = min(gonio_new2)/2;
            gonio2 = [0; angle_half; gonio2];
            gonio_new2 = [0; angle_half; gonio_new2];
            force2 = [coeffs_force(1); (angle_half*coeffs_force(2)) + coeffs_force(1); force2];
        ...
        common_force1_gonio = spline(gonio_new1, force1, average_angle_array);
        common_force2_gonio = spline(gonio_new2, force2, average_angle_array);
        average_force_gonio = (smooth(common_force1_gonio,smoother) + smooth(common_force2_gonio,smoother)) / 2;
        ...
        if plot_us
            plottitle = horzcat('Force vs gonio 2trials fit_avg, ', subject_id, ' (SOL_GMMTJ_GMFAS)');
            figure('Name',plottitle);
            plot(gonio1, force1)
            hold on
            plot(gonio2, force2)
            plot(average_angle_array,common_force1_gonio)
            plot(average_angle_array,common_force2_gonio)
            plot(average_angle_array,average_force_gonio)
            ylabel('Force (N)')
            xlabel('Gonio angle (deg)')
            title(plottitle)
            legend('Gonio1 raw','Gonio2 raw','Gon-force 1 spline','Gon-force 2 spline','Gon-force avg','Location','Northwest');
            print(horzcat('data_plots/',plottitle),'-dpng')
        end

    else
        %% nargin == 9, one trial submitted %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        force1 = varargin{1};
        gonio1 = varargin{2};
        %angle1 = varargin{3};
        displ1 = varargin{4};
        ...
        % reshape and average force across common angle array
        average_force_gonio = smooth(spline(gonio_new1, force1, average_angle_array),smoother);
        ...
        if plot_check && plot_conversion
            plottitle = horzcat('Gonio-force fit/avg, ', subject_id, ' (SOL/GMMTJ/GMFAS)');
            figure('Name',plottitle);
            plot(gonio1, force1)
            hold on
            plot(average_angle_array,average_force_gonio)
            ...
            xlabel('Gonio angle (deg)')
            title(plottitle)
            legend('Gonio raw','Gon-force spline','Location','Northwest');
        end
    ...
    %% output
    if length(emg_gm1) > 1 % 1 = empty: call comes from create_angles_passive, EMG data not needed
        output_array = [average_force_gonio average_angle_array average_displ_gonio average_torque_gonio average_emg_gm_gonio average_emg_gl_gonio average_emg_sol_gonio];
    else
        output_array = [average_force_gonio average_angle_array average_displ_gonio average_torque_gonio];
    end
