...
function noraxon_resampled = read_noraxon_knee(noraxonfile, finalfreq, side, trial_name)
    ...
    while max(norm_angle) > 340
        norm_angle = norm_angle - 360;
    end
    while max(norm_angle) < -180
        norm_angle = norm_angle + 360;
    end
    ...
    if plot_individual
        ...
        legend('Direction/100','Angle','Torque, mV/4','Velocity*10','direction changes','location','northeast')
